...
// Desktop (fine pointer + viewport >= 900px): the pixel field layers follow the pointer
// while it is inside the hero (back 9px, mid 20px, front 31px) and drift back to center on leave.
// Mobile (coarse pointer or viewport < 900px): the pixel field is hidden via CSS; only the
...
function initHeroMotion(): void {
  ...
  const BG_MAX = 56;
  const layers = Array.from(document.querySelectorAll<HTMLElement>('[data-px-layer]'));
  const depths = layers.map((layer) => Number(layer.dataset.pxDepth ?? 0));
